...
The **paint plane** is an overlay (`position: absolute; inset: 0`) laid out at the width the size plane won. It carries the content the reader sees, the room, and the occupant, and it may place them however the alignment asks, because nothing it does can reach back into the sizing. Whether the room is taken is the only state a run has: `occupant` is what stands in it, and `occupied` says it is taken by something the run does not hold — a control floating over the run's end, which cannot nest inside the content it floats over, so the run makes way for it without holding it. With the room free, the content honours the alignment across the whole run. With it taken, the plane becomes a flank, the content, and the room, whose trailing track floors at the reserve. The content then sits at
...
Three costs, all of them contracts the caller keeps and nothing checks. A fitted run renders its content twice, so it must measure the same both times: a phrase rather than a subtree carrying `id`s, and one whose wrapping does not depend on which copy it is. A scrolling run pays nothing here, its ask being the room alone — which is also why the recorder's strip of chips, a subtree rather than a phrase, is a scrolling run and not a fitted one. The `reserve` must be the largest form the end can hold, since an occupant larger than it overflows the room rather than growing it. And the run now claims `position: relative` on itself and `position: absolute; inset: 0` on its overlay — layout plumbing on elements the run owns, which paints nothing and states no length, but which is more than the "tracks and areas only" the primitive started with.
