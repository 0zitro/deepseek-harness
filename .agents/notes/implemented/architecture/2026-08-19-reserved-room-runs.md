...
The **size plane** is the run itself: two hidden exemplars — a ghost of the content and the reserve — in two `max-content` tracks. They are the only things in flow, so the run asks for the content plus the room, always. An occupant is not an input to that ask, because it is not in the plane at all. This is stronger than measuring the occupied and unoccupied states and finding them equal: there is one state.

The **paint plane** is an overlay (`position: absolute; inset: 0`) laid out at the width the size plane won. It carries the content the reader sees, the room, and the occupant, and it may place them however the alignment asks, because nothing it does can reach back into the sizing. With no occupant, the content honours the alignment across the whole run. With one — read straight off the DOM with `:has(> .occupant)`, which is the only state a run has — the plane becomes a flank, the content, and the room, whose trailing track floors at the reserve. The content then sits at
...
- **Reserve symmetrically, a hidden copy in each flank**: centres at every width, but the floor becomes `L + 2R` — Priority 71.17 would become 104.56, taken from the wider columns.
...
Three costs, all of them contracts the caller keeps and nothing checks. The content renders twice, so it must measure the same both times: a phrase rather than a subtree carrying `id`s, and one whose wrapping does not depend on which copy it is. The `reserve` must be the largest form the end can hold, since an occupant larger than it overflows the room rather than growing it. And the run now claims `position: relative` on itself and `position: absolute; inset: 0` on its overlay — layout plumbing on elements the run owns, which paints nothing and states no length, but which is more than the "tracks and areas only" the primitive started with.
...
`ScrollingRun` keeps one plane. Overlap has no meaning inside a scroller — the room must be part of the scroll content to be reachable at the end of it — so the paint plane does not transfer, though the size plane would if its outer ask ever needed pinning. A planned 2D run does transfer: the size plane becomes a 3×3 of hidden exemplars, contributions add per axis by the same disjointness, and the overlay runs the same per-axis templates with an independent switch per reserved edge.
