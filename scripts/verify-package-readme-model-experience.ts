...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-keybindings': { kind: 'none', reason: 'Browser-side input routing and its settings section; a keystroke reaches an action handler, never a model request.' },
  'packages/client/ui-overlay': { kind: 'none', reason: 'Browser-side presentation-only overlay state; registers nothing model-facing.' },
  'packages/client/ui-stock-actions': { kind: 'none', reason: 'Browser-side action registrations over existing composer behaviour; the actions it binds own whatever reaches a model.' },
