...
import { createSnapshotStore, shallowEqual, type SnapshotStore } from '@deepseek-ai/dsh-client-store'
// Type-only: pulls the settings-scope Context merge (ctx.settingsScope) and
// the SettingsScope type the durable list binds to.
...
function bindBindings(host: SettingsScope<KeybindingsSettings>, ctx: Context): BindingsScope {
  ...
  ctx.effect(() => host.subscribe(adopt), 'ui-actions: adopt the stored list')
  adopt()
  ...
  const readable = (): readonly KeybindingOverride[] | undefined => {
    ...
    if (previous === undefined) {
      ctx.logger.error('ui-actions: the stored keybindings are unavailable; the change was not saved')
    }
    return previous
  }
...
export function apply(ctx: Context): void {
  ...
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'ui-actions: dictionaries')
