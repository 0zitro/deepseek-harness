...
在 Host 上注册一个命名空间是必要条件，却并不充分：网关只对一组明确列出的命名空间提供服务，不在其中的命名空间既不会出现在任何描述结果里，写入也会被一律拒绝。已绑定的 scope 会在第一次读到不含该命名空间的描述结果时，以一条指名该命名空间的错误上报此事，且只上报一次——否则这个组合错误只会表现为：偏好设置怎么改都活不过一次重新加载。远程浏览器按设计以 memory 模式绑定，从不上报。
