...
`Table` 持有一个网格的各列、列间的通道，以及移动通道的那些边界；站在其中的一切都属于消费方，按 `tableColumnLine` 给出的线来摆放。单元格用 `data-table-column` 说明自己属于哪一列，表头再加上 `data-table-heading`，下限策略正是据此找到要测量的东西。`TableGroup` 通过 subgrid 把连续若干行放在表格自己的轨道上，于是任何依附于该组的东西——挂在它下面的条带、横贯它的一条线——都有一个高度合适的东西可以依附：一个网格行的高度取决于其中最高的东西，而那很少是条带真正所属的那些单元格。

样式约定分为两个层面，而这一划分就是它的全部。表格**会读回**的东西是数据，经由 props 传入：有哪些列、每列占多少份额、以及一列不可被拖到多窄。份额不能住在 CSS 里，因为拖动会把它换算成像素，而表格将在从未见过它的情况下做这次换算。表格**永不读取**的东西是呈现，留在 CSS 里，通过每个部件一个类、以及每个部件以 `data-` 属性公布的状态来触及。夹在两者之间的，是少数几个表格必须知道、却并无主张的长度——通道的宽度、行与行之间的间隙——它们是注册过的自定义属性（`@property`，因此一个并非长度的值会回退到初始值，而不是连带把轨道模板一起拖垮），消费方可以在任意祖先上设置。每条通道还会先读取自己那条带索引的覆盖值，再退回到共享宽度，于是需要更宽的那一条通道——因为行自己的控件坐落其中——只需一条声明，而不必让表格为此长出一个 prop。

表格自身的规则住在一个级联层里，因此消费方任何未分层的规则都会获胜，无论其特异性如何：一个去猜特异性的库，就是一个会被消费方对着打的库。例外是轨道模板，它由表格以内联方式设置，因为它是从 props 计算出来的——样式表若不把数据再陈述一遍就写不出它；内联同时也让它避开了任何会破坏这套算术的规则。

下限有两个，回答的是不同的问题。列的轨道以它自己的 min-content 兜底，因此容不下各列的面板会滚动，而不是把它们裁掉。而**拖动**以 `floorOf` 所说的为下限，其默认值是该列各单元格在最窄时测得的宽度——即一列展示自己的内容——并且可以更为节省：行内单元格会裁切或滚动自身内容的表格，可以声明只有表头才把列撑开，于是一个长值无法为本有去处的内容去争夺宽度。`minContentWidth` 被导出以便书写这样的策略，并且它是测量而非假定，因为以数字写死的下限只是对某种字体的猜测。边界是一个可获得焦点的 `separator`，并可用方向键移动，因为指针拖动并不是人人都能用来陈述宽度的方式。
