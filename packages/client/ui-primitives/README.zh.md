...
行（run）是这样一段内容：它在两端中的任意一端为时有时无的占位者预留空间——列标题上的排序标记、录制条上方的确认控件、数字字段上的箭头。这块空间以元素而非长度表述：调用方传入该端所能容纳的最宽形态，由引擎去测量，因此这里没有任何规则携带数字，而一个多出排名的标记也就自动预留了带排名标记所需的宽度。每一端都是一个 `RunRoom`——它的 `reserve`、此刻站在其中的 `occupant`，以及用于表示这块空间被行并不持有的某物占据的 `occupied`：比如浮在该端之上、无法嵌套进自己所浮盖内容里的控件，行只为它让路而不持有它。`FittedRun` 就地阅读其内容，`ScrollingRun` 则通过滚动来阅读；两者在 RTL 下都自然镜像、不需要任何与方向相关的规则，也都能在 `align` 提供的每一种模式下守住各自的几何关系。

在两端都预留，正是让内容相对其中一端的占位者保持居中的办法。每块预留空间都站在自己那一侧的余量里，而不是独占一条轨道，因此某一侧的余量以它所持有的东西兜底，两块形态相同的预留空间也就以相同宽度兜底：只要还有富余，内容就守住行自身的中心，只有在没有富余时才让步，并且没有任何长度对此作出规定。无人占据的前端预留空间不绘制任何东西，只花掉自己的宽度。而这份宽度就是要权衡的代价——两端都预留的行，其下限是内容加上两块预留空间而不是一块，这正是列标题只在末端预留、而列内字段（其下限早已由该列的表头定死）可以两端都预留的原因。

行所索要的宽度是被声明出来的，绝不是从它所显示的东西上读出来的。`exemplar` 把预留空间早已享有的待遇给了内容：当内容无法测量自身时，它以什么来测量。控件就无法测量自身——数字输入框报告的是它 `size` 属性的宽度，而不是它取值的宽度——会变化的取值同样如此：测量当下显示的东西，等于把一个随数据移动的宽度交给行。当每一个 exemplar 要么被声明、要么默认取用某个惰性的东西，一个 exemplar 不变的行就无法改变宽度，无论其内部发生什么，持有它的列也就无法移动。

有两条由调用方遵守、无人核查的约定。exemplar 必须是惰性的，并且必须是它那块区域所能容纳的最大形态：行渲染它只为测量，因此作为 `reserve` 传入的控件就是第二个控件，而比 `reserve` 更大的占位者会溢出这块空间，而不是把空间撑大。此外，未获 `exemplar` 的 fitted 行会把内容渲染两次，其中一次隐藏起来以供测量，因此该内容两次必须测得一样：应是一段短语，而不是带 `id` 的子树。滚动行两项都不付，也不接受 `exemplar`：它索要的只有那些预留空间——这也正是一条可以任意长的按键块条要用滚动行的原因。滚动行的预留空间还会随占位者的来去带走可滚动范围，因此在意读者所处位置的调用方应把该位置记下来并还回去。fitted 行为何需要两个平面才能做到这些：见[预留空间笔记](../../../.agents/notes/implemented/architecture/2026-08-19-reserved-room-runs.md)。
