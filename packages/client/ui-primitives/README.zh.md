...
两个平面并非图省事的做法。在 grid 中，两个项的内在贡献只有在其轨道彼此不相交时才相加；因此若行索要的正好是内容加预留空间，而内容与 reserve 又同为这份索要的来源，它们的跨度就会铺满整个行，内容的跨度要么紧贴起始端、要么紧接在预留空间之后——绝不会居中。重叠是绘制层面的事实，不相交是尺寸层面的事实；出路就是不再让同一个元素同时干这两件事，这也正是 fitted 行中内容的影子恰为 reserve 之镜像的原因。它的代价是内容渲染两次，因此两次必须测得一样：应当是一段短语，而不是带 `id` 的子树，并且其换行不应取决于自己是哪一份副本。滚动行在这一项上不付任何代价，因为它索要的只有预留空间。两种绘制平面都用 grid 而非 flex，是为了 flex 没有的一条性质——`1fr` 轨道即 `minmax(auto, 1fr)`，轨道的最小尺寸与它在富余空间中的份额彼此独立；而 flex 项的基础尺寸会计入容器的 `min-content`，富余就被记到了下限上。
