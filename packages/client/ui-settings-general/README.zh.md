...
面板是锚定的，而不是居中的：视口居中的是一个基准宽度的盒子，面板从这个盒子的左边缘起始、向右生长。因此无论打开哪个分区，导航栏都待在同一个位置，而让面板自身居中做不到这一点——宽度一变，它的左边缘就会移动，导航栏也跟着移动，位移是宽度差的一半。内容放不进基准内容列的分区改用宽面板；这项选择和导航图标一样归外壳所有，因为注册项携带的是导航身份，而面板的几何形状属于外壳持有的界面框架。

选项面板始终保留滚动条的槽位，无论其中是否真的绘制了滚动条，这样当某个分区自身内容长到超出面板时，该分区仍保持它布局时的宽度。若不这样，把滚动条画在流内的引擎会在内容高度越过面板的那一刻从分区里取走滚动条的宽度，宽到足以看出差别的分区便会因自身高度的变化而重排；而把滚动条叠加绘制的引擎从不如此，于是同一个分区看上去在不同浏览器里布局各异。
