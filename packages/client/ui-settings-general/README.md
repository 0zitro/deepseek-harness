...
The panel is anchored rather than centered: a base-width box is what the viewport centers, and the panel starts at that box's left edge and grows rightward. The nav rail therefore sits in the same place whichever section is open, which centering the panel itself could not give — a width change would move its left edge, and the rail with it, by half the difference. A section whose content does not fit the base content column takes the wide panel; that choice belongs to the shell, like the nav glyph, because a registration carries the nav identity and the panel's geometry is chrome the shell owns.

The options pane holds its scrollbar gutter whether or not a bar is drawn in it, so a section keeps the width it laid out at when its own content grows past the pane. Without that, an engine that draws its scrollbar in flow takes the bar's width out of the section at the moment the content crosses the pane's height, and a section wide enough to notice reflows against a change in its own height — while an engine that overlays its scrollbars never does, so the same section appeared to lay out differently per browser.
